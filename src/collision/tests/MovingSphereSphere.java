...
public class MovingSphereSphere {
	 ...
	 */
	public CollisionData testMovingSphereSphere(Entity s0, Entity s1)
	{
		Vector v0 = s0.speed;
		Vector v1 = s1.speed;
		...
		// Spheres initially overlapping so exit directly
			rData.t = 0.0f;
			System.out.println("Intersection, initially overlapping");
		...
		float b = v.dot(s);
		if (b > 0.01f) {
			System.out.println("Spheres not moving towards eachother");
	...
	public CollisionData testMovingSphereSphere2(Entity s0, Entity s1, Vector v0, Vector v1)
	{
		// Expand sphere s1 by the radius of s0
		float r = s1.getBoundingRadius();
		s1.setBoundingRadius(r+s0.getBoundingRadius());
		...
		CollisionData rData = rTest.IntersectRaySphere(s0.pos, v.multiply(1/vlen), s1);
		rData.collision = rData.t <= vlen;
		s1.setBoundingRadius(r);
		return rData;
	}
}
